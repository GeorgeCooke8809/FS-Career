...
| `timezone`  | String     | No       |          | IANA timezone name (e.g. `Europe/London`), used for local-time display/PA scheduling. |
...
| `network_model` | String(16) | Yes  | Check: `hub_and_spoke`/`point_to_point` | Real-world route network structure. Populated for the 325 airlines that have at least one route; `NULL` for the ~400 route-less airlines. See `data/seed/seed_network_model.py`. |
...
The fact table — each row is one specific scheduled service: an airline flying a given flight number, on a given aircraft type, between two airports.
...
| `schedule_synthetic`     | Boolean   | No       | Default `True`                | `True` if `duration_minutes` was generated rather than sourced from a real timetable. |
...
- `routes` is bulk-seeded from OpenFlights' route network data (real airline → route → aircraft-type triples), matched against our own `airlines`/`airports`/`aircraft` tables — airlines/airports we don't have are dropped, but unmatched/generic equipment codes (e.g. KLM's rows mostly saying `"330"`/`"737"` instead of a real variant code) fall back to a representative variant from our `aircraft` table rather than being dropped, flagged via `aircraft_icao_type_synthetic`. `flight_number` and `duration_minutes` are always synthesized, since no free source has bulk real-world schedule data (that's commercial OAG/Cirium territory) — see `memory/project_routes_import.md` for the full process and coverage numbers.
- `airlines.network_model` was researched and set per-airline for the 325 route-having carriers — see `data/seed/network_model.json`/`seed_network_model.py`. Because `routes` origin/destination pairs come from real OpenFlights schedule data (not fabricated), route topology already reflects each airline's real network shape by construction; a post-hoc airport-concentration check (normalized entropy of per-airline airport degree) found no systemic contradiction between the `network_model` classification and the actual route data, so no route rows were altered to "fit" the model.
- Every `hub_and_spoke` airline's routes are guaranteed reciprocal — for every `A -> B` there's a `B -> A` — since a hub-and-spoke passenger network is unusable if you can't fly back. 605 one-way gaps (an OpenFlights sourcing artifact) were backfilled by `data/seed/fill_hub_and_spoke_return_routes.py`, mirroring the outbound leg's aircraft type/distance/duration and synthesizing a new flight number. `point_to_point` airlines weren't touched — see `memory/project_network_model.md`.
